...
## 4.消费者
设计上允许重复消费（由消费者保证）
消息消费以组的模式开展，一个消费组内可以包含多个消费者，每一个消费者可以订阅多个主题。
消息组之间有集群模式和广播模式两种消费模式。
...
消息服务器和消费者之间的消息传递有2种方式：推模式和拉模式
拉模式：消费端主动发起拉消息请求
推模式(推荐)：消息到达消息服务器后，推送给消息消费者。推模式是基于拉模式

消息拉取，如果没有找到消息的情况下，有2种机制：
1.服务端长轮询： 如果开启，即每5s尝试一次，判断新消息是否到达，是，会调用重新拉取消息
2.消息到达，通过监听器的方式立即调用重新拉取消息


* 消息队列【负载与重新分配】
消费组数量发生变化 rebalance？新的消费者加入
消息队列负载机制遵循一个原则：一个消息队列同一时刻只允许被一个消费者消费，一个消费者可以消费多个消息队列。

RebalanceService线程每20s对消费者订阅的主题进行一次队列重新分配

过程：
根据 当前topic下的消费队列集合、消费组下的所有客户端id集合、分配策略，得到前客户端id所分配的新的负载队列。
判断 新分配的负载队列 和当前负载的消费队列 是否发生变化，从而进行停止消费或新建拉取任务进行消费操作。
...
Q: 梳理一下定时任务，结合deepwiki看一下

Q: consumer和producer的关系，谁拥有谁？
consumer、instance
...
* 顺序消息 （185 - 202）
RocketMQ支持局部顺序消息消费，也就是保证同一个消费队列上的的消息顺序消费。
不支持全局顺序消费，如果要实现某一个主题的全局顺序消息，可以将主题的队列设置为1（牺牲高可用性）
